...
if (test_no == 0)
  ...
  % Check that the variables have the same names and dimensions
  ...
  for ii = 1:num_elements
    ...
    if ~strcmp(desc_dds_ref.variable(ii).type, desc_dds_test.variable(jj).type)
...
elseif (test_no >= 1) & (test_no <= 4)
  ...
  if size(stj) ~= size(serial_time_jd(ra))
    error('!! timenc failed: produces wrong size of serial_time_jd')
  else
    if size(stj) > 0
      dd = abs(stj - serial_time_jd(ra)) + ...
	   abs(isnan(stj) - isnan(serial_time_jd(ra)));
      if max(dd(:)) > 0
...
elseif (test_no >= 6) & (test_no <= 19)
  ...
  eval(cmd)
  if size(av) ~= size(att_val)
    error(['!! attnc failed: finds wrong number of attributes for ' var{ii}])
  else
    for jj = 1:length(av)
      if ischar(av{jj}) & ischar(att_val{jj})
	if ~strcmp(av{jj}, att_val{jj})
	  error(['!! attnc failed: 1: attribute ' num2str(jj) ' of ' ...
		 var{ii} ' wrong'])
	end
      elseif isnumeric(av{jj}) & isnumeric(att_val{jj})
	if size(av{jj}) == size(att_val{jj})
	  switch test_type
	    case 'netcdf'
	     dd = abs(av{jj} - att_val{jj}) + ...
		  abs(isnan(av{jj}) - isnan(att_val{jj}));
	     if max(dd(:)) > 0
	       error(['!! attnc failed: 2: attribute ' num2str(jj) ' of ' ...
		      var{ii} ' wrong'])
	     end
	   case 'opendap' % allow for possible rounding error.
	     dd1 = abs(isnan(av{jj}) - isnan(att_val{jj}));
	     if max(dd1(:)) > 0
